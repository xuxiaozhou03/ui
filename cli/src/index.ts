...
const generateStyle = (wxss: string) =>
  wxss.replace("@import '../common/index.wxss';", "");

const generate = (packageName: string, content: string) => {
  const outputDir = path.join(__dirname, "..", "output");
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir, { recursive: true });
  }
  fs.writeFileSync(path.join(outputDir, packageName + ".json"), content);
};

packages.forEach((packageName) => {
  const content = getPackageFileContent(packageName);

  const style = generateStyle(content.wxss);
  const template = generateTemplate(content.wxml);
  const ts = content.ts;
  generate(packageName, JSON.stringify({ style, template, ts }, null, 2));
});
