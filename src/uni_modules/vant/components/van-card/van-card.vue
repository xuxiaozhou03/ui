<template>
  <view :class="['custom-class', 'van-card']">
    <view :class="bem('card__header', { center: centered })">
      ...
      >
        <view>
          <view v-if="title" class="van-card__title title-class">{{
            title
          }}</view>
...
const emit = defineEmits(["click-thumb"]);

const integerStr = ref("");
const decimalStr = ref("");
...
function updatePrice(price: string | undefined) {
  if (!price) {
    integerStr.value = "";
    decimalStr.value = "";
    return;
  ...
  integerStr.value = priceArr[0];
  decimalStr.value = priceArr[1] ? `.${priceArr[1]}` : "";
}

function onClickThumb() {
  emit("click-thumb", props.thumbLink);
}
</script>
<style>
.van-card {
